...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/10/12 13:51:59 by lregenar          #+#    #+#             */
/*   Updated: 2020/10/25 16:28:24 by lregenar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_slablen(t_all *s, char *line, int key)
{
	int	i;
	int	count;
	...
	if (key == 1 && s->map.x != 0 && s->map.x != count)
		return (count);
	return (count);
}
...
char	*ft_slab(t_all *s, char *line, int *i)
{
	...
	if (!(slab = malloc(sizeof(char) * (ft_slablen(s, line, 1) + 1))))
		return (NULL);
	j = 0;
	while (line[*i] != '\0')
	{
...
int		ft_map(t_all *s, char *line, int *i)
{
	...
	s->map.tab = tmp;
	s->map.y++;
	if ((s->map.x = ft_slablen(s, line, 1)) == -1)
		return (-13);
	return (0);
}
